...
class UserRecoveryForm extends CFormModel {
	...
	public function rules()
	{
		return array(
			...
			array('login_or_email', 'match', 'pattern' => '/^[A-Za-z0-9@.-\s,]+$/u','message' => UserModule::t("Некорректные символы в логине (или email'e). Допускаются только @, дефис, точка, цифры и латинские буквы.")),
	...
	{
		return array(
			'login_or_email'=>UserModule::t("Логин или email"),
		);
	}
	...
	public function checkexists($attribute,$params) {
		if(!$this->hasErrors())  // we only want to authenticate when no input errors
		{
			...
			if($user===null)
				if (strpos($this->login_or_email,"@")) {
					$this->addError("login_or_email",UserModule::t("Email набран неверно."));
				} else {
					$this->addError("login_or_email",UserModule::t("Логин набран неверно."));
				}
		}
	}
